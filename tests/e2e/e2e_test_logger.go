...
// can construct a logger without importing the e2e package (which would be a cycle).
func init() {
	common.NewTestLogger = NewTestLogger
}

// NewTestLogger creates a new TestLogger and returns it as a common.Logger.
...
// FIX: previously this passed `elapsed` as the first Sprintf arg AND tried to
// pass `args...` after it, which Go rejects when format only has one verb.
// We now format the prefix separately, then format the user's message.
